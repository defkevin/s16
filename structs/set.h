...
template <typename T>
class set { // set is a container adapter
...
public:
	...
	bool remove(iter it);
	iter insert(T x); // insert x and return the iter for x. Return end() if already in set
...
template <typename T>
typename set<T>::iter set<T>::begin() {
	return iter(c_.head());
}

template <typename T>
typename set<T>::iter set<T>::end() {
	return iter(nullptr);
}

template <typename T>
typename set<T>::iter set<T>::find(T x) {
	iter i = begin();
	while(i!=end()){
		if(i.value() == x){
			return i;
...
template <typename T>
typename set<T>::iter set<T>::insert(T x) {
	iter i = find(x);
	if(i!=end()){ // WE FOUND x
		return i;
	}
